...
// Ensure fetch is available in global scope (Node 18+ has native fetch, fallback to node-fetch)
if (!globalThis.fetch) {
  globalThis.fetch = fetch;
}

// Global session registry to share active transports across handlers in warm serverless instances
...
/**
 * Creates and configures a GitHub MCP Server instance with all tools registered.
 */
    ...
    try {
      ...
      switch (request.params.name) {
        ...
        case "get_pull_request_files": {
          ...
          return {
            content: [{ type: "text", text: JSON.stringify(filesList, null, 2) }],
          };
        }
