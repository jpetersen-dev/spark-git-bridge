...
import { notionTools, handleNotionTool } from "./services/notion.js";
 ...
 * @param {string[]} [options.services=['github', 'telegram', 'notion']] Lista de servicios a habilitar
 * @returns {Server} Instancia del servidor MCP
 */
export function createMcpServer(options = {}) {
  const enabledServices = options.services || ["github", "telegram", "notion"];
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    ...
    if (enabledServices.includes("notion")) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    // 2.2 Si es una herramienta de Notion
    if (name.startsWith("notion_")) {
      if (!enabledServices.includes("notion")) {
        throw new Error(`Notion service is not enabled on this route.`);
      }
      const notionResult = await handleNotionTool(name, args);
      if (notionResult) return notionResult;
    }

    // 2.3 Si es una herramienta de GitHub
    if (enabledServices.includes("github")) {
